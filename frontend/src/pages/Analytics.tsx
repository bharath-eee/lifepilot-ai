  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div>
        ...
        <p className="text-slate-400 text-xs mt-1">Productivity metrics and auto-extracted bill categories summaries.</p>
      </div>

      <div className="grid gap-6 md:grid-cols-3">
        {/* Core Stats */}
        <div className="glass-panel p-6 rounded-2xl border-glassBorder flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-500 font-mono">TOTAL INVOICED</p>
            <p className="text-2xl font-bold text-white mt-1">₹49,989</p>
          </div>
        ...
        <div className="glass-panel p-6 rounded-2xl border-glassBorder flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-bold text-white mt-1">42 / 48</p>
          </div>
      ...
      {/* Category chart */}
      <div className="glass-panel p-6 rounded-3xl border-glassBorder space-y-6">
        <h2 className="text-sm font-bold text-white font-mono">EXPENSES BY CATEGORY</h2>
        <div className="space-y-4">
          {categories.map((cat, idx) => (
            <div key={idx} className="space-y-1.5">
              <div className="flex justify-between text-xs font-semibold">
                <span className="text-slate-300">{cat.name}</span>
                <span className="text-white">₹{cat.amount} ({cat.percentage}%)</span>
              </div>
              <div className="w-full bg-white/5 h-2.5 rounded-full overflow-hidden border border-glassBorder/30">
