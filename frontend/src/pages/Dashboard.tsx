...
export const Dashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      {/* AI Planner Banner */}
      <div className="relative p-6 md:p-8 rounded-2xl glass-panel overflow-hidden border-glassBorder shadow-sm">
        <div className="absolute top-0 right-0 -mt-10 -mr-10 w-48 h-48 bg-gradient-to-br from-neonBlue/5 to-neonPurple/10 rounded-full blur-3xl" />
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center space-x-3 bg-slate-50 py-2.5 px-4 rounded-2xl border border-glassBorder self-start md:self-auto">
            <Clock className="w-5 h-5 text-neonBlue animate-pulse" />
            <div className="text-left font-mono">
              <p className="text-xs text-slate-500">SYSTEM TIME</p>
              <p className="text-sm font-semibold text-slate-900">03-JULY-2026 23:20</p>
            </div>
          </div>
        </div>

        {/* Dynamic AI Suggestions Panel */}
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 border-t border-glassBorder pt-6">
          <div className="flex items-start space-x-3 bg-slate-50/50 p-4 rounded-xl border border-glassBorder">
            ...
            <div>
              ...
              <p className="text-sm text-slate-700 mt-0.5">Electricity bill of <span className="font-semibold text-neonOrange">₹1,240</span> is due tomorrow.</p>
            </div>
          </div>
          <div className="flex items-start space-x-3 bg-slate-50/50 p-4 rounded-xl border border-glassBorder">
            ...
            <div>
              ...
              <p className="text-sm text-slate-700 mt-0.5">Free slot detected between 2 PM - 4 PM today for interview prep.</p>
            </div>
          </div>
          <div className="flex items-start space-x-3 bg-slate-50/50 p-4 rounded-xl border border-glassBorder sm:col-span-2 lg:col-span-1">
            ...
            <div>
              ...
              <p className="text-sm text-slate-700 mt-0.5">Microsoft sent interview invitation. Replying required.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="glass-panel rounded-2xl p-6 flex flex-col h-[400px]">
          <div className="flex justify-between items-center mb-6">
            ...
            <button className="p-1.5 rounded-lg bg-slate-50 border border-glassBorder text-slate-500 hover:text-slate-900 transition-all">
            ...
            {tasks.map(task => (
              <div 
                ...
                className={`flex items-start justify-between p-3.5 rounded-xl border transition-all duration-300 ${
                  task.completed 
                    ? 'bg-slate-50/40 border-glassBorder opacity-55' 
                    : 'bg-slate-50 border-glassBorder hover:border-neonPurple/30'
                }`}
              >
        ...
        <div className="glass-panel rounded-2xl p-6 flex flex-col h-[400px]">
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center space-x-2">
              ...
              <h2 className="text-lg font-bold text-slate-900">Important Mail</h2>
            </div>
            <span className="text-xs font-mono py-0.5 px-2 bg-neonBlue/10 text-neonBlue rounded-full border border-neonBlue/20">
              3 Urgent
            </span>
          </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-3 pr-1">
            {emails.map(email => (
              <div 
                key={email.id} 
                className="bg-slate-50 border border-glassBorder hover:border-neonBlue/30 p-3.5 rounded-xl transition-all duration-300 flex justify-between items-start"
              >
                <div className="space-y-1">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-[10px] text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full">{email.category}</span>
                  </div>
                  <p className="text-sm font-medium text-slate-700 truncate max-w-[200px]">{email.subject}</p>
                </div>
        ...
        {/* Bills Widget */}
        <div className="glass-panel rounded-2xl p-6 flex flex-col h-[400px] md:col-span-2 lg:col-span-1">
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center space-x-2">
              <Receipt className="w-5 h-5 text-neonGreen" />
              <h2 className="text-lg font-bold text-slate-900">Upcoming Bills</h2>
            </div>
            <span className="text-xs text-slate-500 font-mono">Unpaid: 2</span>
          </div>
            ...
            {bills.map(bill => (
              ...
              >
                <div className="space-y-0.5">
                  <p className="text-sm font-bold text-slate-800">{bill.vendor}</p>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <div className="glass-panel p-6 rounded-2xl flex justify-between items-center">
          <div className="space-y-1">
            ...
            <p className="text-2xl font-bold text-slate-900">₹3,489</p>
